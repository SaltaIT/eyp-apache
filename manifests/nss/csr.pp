define apache::nss::csr (
  ...
  exec { "nss csr ${certdb} ${keysize} ${aliasname} ${name} ${country} ${state} ${locality} ${organization} ${cn}":
  ...
  file { "${apache::params::baseconf}/ssl/${aliasname}.csr":
    ...
    group   => 'root',
    mode    => '0644',
    require => Exec["nss csr ${certdb} ${keysize} ${aliasname} ${name} ${country} ${state} ${locality} ${organization} ${cn}"],
  }
